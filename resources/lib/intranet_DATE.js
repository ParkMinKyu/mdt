...
function gfnGetHolidays(nYear)
{
 aHoliday = new Array();
 /////// 음력 체크
 var holiday1 = new Date(gfnLunar2Solar( "0" + (nYear) + "0101" ));
 var holiday2 = new Date(gfnLunar2Solar( "0" + (nYear) + "0408" ));
 var holiday3 = new Date(gfnLunar2Solar( "0" + (nYear) + "0815" ));
 aHoliday[0] =  {
	 allDay : true,
	 title  : "설날", 
	 start : new Date(holiday1.setDate(holiday1.getDate()-2)),
	 end : new Date(holiday1.setDate(holiday1.getDate()+3)),
	 color:'red'};
 aHoliday[1] =  {
	 allDay : true,
	 title  : "석가탄신일", 
 ...
 aHoliday[2] =  {
	 allDay : true,
	 title  : "추석", 
	 start : new Date(holiday3.setDate(holiday3.getDate()-1)),
	 end : new Date(holiday3.setDate(holiday3.getDate()+3)),
	 color:'red'};
 aHoliday[3] = {
	 allDay : true,
	 title  : "신정", 
	 start : new Date(nYear,"00","01"),
	 color:'red'};
 aHoliday[4] = {
	 allDay : true,
	 title  : "삼일절", 
	 start : new Date(nYear,"02","01"),
	 color:'red'};
 aHoliday[5] = {
	 allDay : true,
	 title  : "근로자의날", 
	 start : new Date(nYear,"04","01"),
	 color:'red'}; 
 aHoliday[6] = {
	 allDay : true,
	 title  : "어린이날", 
	 start : new Date(nYear,"04","05"),
	 color:'red'}; 
 aHoliday[7] = {
	 allDay : true,
	 title  : "현충일", 
	 start : new Date(nYear,"05","06"),
	 color:'red'};  
 aHoliday[8] = {
	 allDay : true,
	 title  : "광복절", 
	 start : new Date(nYear,"07","15"),
	 color:'red'};   
 aHoliday[9] = {
	 allDay : true,
	 title  : "개천절", 
	 start : new Date(nYear,"09","03"),
	 color:'red'};   
 aHoliday[10] = {
	 allDay : true,
	 title  : "한글날", 
	 start : new Date(nYear,"09","09"),
	 color:'red'};   
 aHoliday[11] = {
	 allDay : true,
	 title  : "성탄절", 
	 start : new Date(nYear,"11","25"),
	 color:'red'};
 // 구정
...
function isHoliday(date){
	...
	for(var i = 0 ; i < aHoliday.length ; i ++){
		var temp = aHoliday[i].start;
		if(temp.getMonth() == m && temp.getDate() == d){
			return true;
		}
	}
	return false;
...
function getHoliday(moment){
    /*for(var i = 0 ; i < aHoliday.length ; i ++){
		...
		if(temp[3] == '설날'){
			$.fullCalendar( 'renderEvent',{title  : temp[3],start  : new Date(temp[0],(parseInt(temp[1])-1),temp[2]),end : new Date(temp[0],(parseInt(temp[1])-1),parseInt(temp[2])+2),color:'red'} );
		}else if(temp[3] == '추석'){
			$.fullCalendar( 'renderEvent',{title  : temp[3],start  : new Date(temp[0],(parseInt(temp[1])-1),temp[2]),end : new Date(temp[0],(parseInt(temp[1])-1),parseInt(temp[2])+2),color:'red'} );
		}else if(temp[3] != null ){
			$.fullCalendar( 'renderEvent',{title  : temp[3],start  : new Date(temp[0],(parseInt(temp[1])-1),temp[2]),color:'red'} );
		}
		
	}*/
    gfnGetHolidays(moment.year());
    return aHoliday;
}
...
function getPayDay(moment){
	...
	return {allDay : true, title  : '월급날',start  : new Date(y,m,d),color:'red'} ;
}